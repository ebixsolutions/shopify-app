...
export default function BoostEshopTrafficManagement() {
  ...
  useEffect(() => {
    if (user.shopify_code || user.token) {
      setIframeSrc(
        `${Iframe}boostTraffic?shopify_code=${user.shopify_code}&token=${user.token}`,
      );
    }
  ...
  return (
    <div className={styles.iframeContainer}>
      ...
      {iframeSrc && (
        <iframe
          src={iframeSrc}
          title="Boost Eshop Traffic Management"
          width="100%"
          height="100%"
          style={{
