...
export default function BoostRepeatCustomerLabels() {
  ...
  useEffect(() => {
    if (user.shopify_code || user.token) {
      setIframeSrc(
        `${Iframe}boostRepeatCustomers?shopify_code=${user.shopify_code}&token=${user.token}`,
      );
    }
  ...
  return (
    <div className={styles.iframeContainer}>
      ...
      {iframeSrc && (
        <iframe
          src={iframeSrc}
          title="Boost Repeat Customer Labels"
          width="100%"
          height="100%"
          style={{
