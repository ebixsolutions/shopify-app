...
export const validateSessionMiddleware = async (request, shop) => {
  ...
  const isPrivateWindow =
    userAgent.includes("Incognito") || userAgent.includes("Private");
        ...
        try {
          ...
          if (user && user.user_id && user.token) {
            ...
            try {
              ...
              console.log(
                "Could not set session (likely private window), continuing with URL data",
              );
            }
          } else {
    ...
    if (user && user.user_id && user.token) {
      ...
      if (!response.error) {
        if (response.msg === "User Not Found" || response.msg === "New User") {
          console.log("User not found, clearing session:", response.msg);
          return { valid: false, error: "User not found", clearSession: true };
        } else {
